...
namespace shared_memory {
    ...
    typedef bip::node_allocator<char, SegmentManager> CharAllocator;
    ...
    template<class PayloadType>  // PayloadType could be a set or int or complex stuct (with strings)
    class MappedValType {
    public:
        MappedValType() { }

        MappedValType(time_t expires_at, const VoidAllocator &void_alloc) : payload(void_alloc),
    ...
    public:
        ...
        bool set(const char *k, const PayloadType &val, bool create_only = true, int expires = 0) {
            ...
            String k_(k, VoidAllocator(segment_->get_segment_manager()));
    ...
    struct BasicSet {
        typedef bip::node_allocator<ValType, SegmentManager> ShmemAllocator;
        typedef bip::set<ValType, std::less<ValType>, ShmemAllocator> Type;
    };

    template<class KeyType, class ValType>
    class MapSet : public Map<KeyType, bip::set<ValType, std::less<ValType>, bip::node_allocator<ValType, SegmentManager>>> {

        typedef bip::set<ValType, std::less<ValType>, bip::node_allocator<ValType, SegmentManager>> PayloadType;
    ...
    public:
        ...
        bool add(const KeyType &k, const ValType &val, int expires = 0) {
            ...
            VoidAllocator alloc_inst(segment_->get_segment_manager());
            auto it = map_->find(k);
            if (it == map_->end()) {
