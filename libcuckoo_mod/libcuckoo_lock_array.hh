...
private:
  ...
  bip::offset_ptr<spinlock> create_segment() {
    bip::offset_ptr<spinlock> arr = traits_::allocate(allocator_, SEGMENT_SIZE);
  ...
  void destroy_segment(bip::offset_ptr<spinlock> arr) {
    traits_::deallocate(allocator_, arr, SEGMENT_SIZE);
  }

  // Emulates the data in `src` into `dst`
  static void emulate_segment(bip::offset_ptr<spinlock>dst, bip::offset_ptr<spinlock>src) {
    for (size_type i = 0; i < SEGMENT_SIZE; ++i) {
  ...
  bip::offset_ptr<spinlock>copy_segment(bip::offset_ptr<spinlock>src) {
    bip::offset_ptr<spinlock>dst = create_segment();
  ...
  typename traits_::template rebind_alloc<bip::offset_ptr<spinlock>> segment_allocator_;
    bip::offset_ptr<bip::offset_ptr<spinlock>> segments_;
